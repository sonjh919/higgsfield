---
name: higgsfield-fx
description: 정지 이미지용 Higgsfield 프롬프트의 FX(시각효과) 레이어를 보강하는 스킬. Impact·Particle·Fluid·Optical·Atmospheric·Magical·Material 7카테고리의 영문 어휘와 구절을 제공. 사용자가 "FX 추가", "이펙트 보강", "타격감 / 파편 / 입자 / 안개 / glow / lens flare / 잔상 추가", "/higgsfield-fx" 등을 말하면 이 스킬을 사용한다. 영상 FX는 OSideMedia higgsfield-motion 프리셋을 우선 참조한다.
user-invocable: true
---

# Higgsfield FX Layer Builder (정지 이미지 전용)

Higgsfield 정지 이미지 프롬프트의 **FX(시각효과) 레이어**만 좁게 담당하는 add-on 스킬. 본 프롬프트(MCSLA / 9요소)는 OSideMedia `higgsfield-prompt` / `higgsfield-image-shots` 또는 `higgsfield-mine`이 만들고, 이 스킬은 그 결과물에 FX 어구를 자연스럽게 얹거나 단독 FX 구절을 생산한다.

- **다루는 것**: 7카테고리(Impact / Particle / Fluid / Optical / Atmospheric / Magical / Material)의 영문 어휘·구절·결합 룰
- **다루지 않는 것**: 영상 FX (→ `higgsfield-motion` 프리셋), 비포토리얼 스타일 변환 (→ `higgsfield-mixed-media`), 본 프롬프트의 Subject·Camera·Action 골격
...
**OSideMedia 매핑** (영상 변환 시 참고): `Explosion` / `Building Explosion` / `Atomic` / `Plasma Explosion` (`higgsfield-motion/SKILL.md`).
...
**OSideMedia 매핑**: `Disintegration` / `Clone Explosion`(영상 변환 시).
...
**OSideMedia 매핑**: `Water Bending` / `Splash Transition`(영상 변환 시).
...
**OSideMedia 매핑**: `Glow Trace` / `Innerlight`(영상 변환 시), `higgsfield-style/SKILL.md`의 Lighting + FX pairing 표.
...
**OSideMedia 자산**: `~/.claude/skills/higgsfield/vocab.md`의 atmospheric lexicon (volumetric / fog / mist / dust haze) 직접 차용.
...
**OSideMedia 매핑**: `Luminous Gaze` / `Innerlight` / `Glow Trace`(영상 변환 시).
...
**OSideMedia 매핑**: `Turning Metal` / `Freezing` / `Ice Rose`(영상 변환 시).
...
- **Color anchor + FX combo** — `cold blue moonlight + faint blue rune glow`처럼 색감과 FX를 한 색계로 묶으면 일관성↑. (`higgsfield-style/SKILL.md` 패턴 차용)
...
## OSideMedia 본 스킬과의 협력

- **영상 요청은 즉시 위임** — "이 장면을 영상으로", "도끼가 부딪히는 순간 뼈가 산산조각" 류의 요청 → `higgsfield-motion`의 명명된 프리셋(`Disintegration`, `Plasma Explosion`, `Glow Trace` 등)으로 안내. 본 스킬은 정지 이미지 전용.
- **단어수 보호** — augment 모드에서 본 프롬프트가 60단어를 넘거나 Cinema Studio 사용 명시 시 **512자 룰** 보호를 위해 FX 추가 전 사용자에게 단축/생략 여부 확인.
- **negative-constraints 룰 준수** — `~/.claude/skills/higgsfield/skills/shared/negative-constraints.md`의 Body/Motion·Texture/Lighting·Temporal/Consistency 룰과 충돌하지 않게 어구 선택. 특히 contradictory motion (Dolly In + Dolly Out) 같은 충돌 패턴을 FX로도 만들지 않을 것.
