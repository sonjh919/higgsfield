...
변경 이력이 궁금할 때는 `/higgsfield-fx-tuner status`, 다른 사람한테 보고서로 공유하고 싶을 때는 `/higgsfield-fx-tuner report`를 입력하세요.

### 작업 예시 (전체 흐름)

1. **프롬프트 생성** — 레퍼런스 이미지를 첨부하면서 자연어로 설명하면 `higgsfield-fx`가 영문 프롬프트를 코드 블록으로 출력
   ```
   [이미지 첨부] 무기에 황금빛 에너지 발광, 머리 제외 신체 아우라
   ```

2. **이미지 생성** — 출력된 영문 프롬프트를 Higgsfield(또는 다른 이미지 모델)에 붙여넣고 결과 확인

3. **피드백** — 결과가 의도와 다르면 한국어로 자연스럽게 한 줄
   ```
   이 glow가 너무 강해 / 좀 더 subtle하게
   ```
   → `higgsfield-fx-tuner`가 자동으로 기록: `📝 피드백 기록됐어요 (1/7)`

4. **반복** — 1~3단계를 여러 번 반복하면서 피드백이 누적됨

5. **임계치 알림** — 7번째 피드백 시 한 번 안내가 뜸
   ```
   💡 7개 모였어요. /higgsfield-fx-tuner review 입력하시면 정리해드려요.
   ```

6. **큐레이션 적용** — `/higgsfield-fx-tuner review` → 변경안 미리보기 → "예" → 본인 로컬의 `higgsfield-fx/SKILL.md` 자동 업데이트 + 자동 백업 + `CHANGELOG.md` 항목 추가

7. **(선택) 보고서 공유** — `/higgsfield-fx-tuner report` → 텍스트 보고서 출력 → 카톡/이메일로 다른 사람에게 복붙

8. **(필요 시) 롤백** — 방금 적용한 변경이 마음에 안 들면 자연어로 "되돌려줘" → 마지막 백업으로 복구
